...
//
	public List<String> getAllItemNameList() throws SQLException{
		List<Integer> allItemIdList =new ArrayList<Integer>();
		...
		try{
			...
			ResultSet resultSet =preparedStatement.executeQuery();
			while(resultSet.next()){
				allItemIdList.add(Integer.parseInt(resultSet.getString("id")));
				allItemNameList.add(resultSet.getString("item_Name"));
			}
		} catch(Exception e){
			e.printStackTrace();
		} finally{
